...
local BrushCursor = Roact.PureComponent:extend("BrushCursor")

function BrushCursor:init(props)
	self.axisFrameRef = Roact.createRef()
	self.cursorRef = Roact.createRef()
	self.cursorFrameRef = Roact.createRef()
end

function BrushCursor:render()
...
function BrushCursor:didMount()
	...
	self.hConn = game:GetService("RunService").Heartbeat:Connect(function(dt) -- I really just want to trigger an update every frame.
		local brushtool = getBrushtool(self)
		...
		if brushtool.mode == "Brush" and brushtool.hit then
...
function BrushCursor:willUnmount()
...
return RoactRodux.connect(mapStateToProps, mapDispatchToProps)(BrushCursor)
