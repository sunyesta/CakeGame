...
function AutoHeightThemedText:doUpdate()
	...
	if self.changedDebounce then return end
	self.changedDebounce = true
	local textSize = Utility.GetTextSize(
		textLabel.Text, textLabel.TextSize, textLabel.Font, Vector2.new(textLabel.AbsoluteSize.X, 9999)
	)
...
function AutoHeightThemedText:didMount()
	local frame = self.frameRef.current
	local textLabel = self.textRef.current
	self.sizeChangedConn = textLabel:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
		-- HAX
